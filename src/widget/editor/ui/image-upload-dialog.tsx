...
export function ImageUploadDialog({ onInsert, onClose }: { onInsert: (url: string, alt: string) => void; onClose: () => void }) {
  ...
  return <div className="fixed inset-0 z-50 grid place-items-center bg-black/30 p-4">
    <div className="w-full max-w-md space-y-3 rounded-lg bg-white p-5 shadow-xl">
      ...
      <Input type="file" accept="image/jpeg,image/png,image/gif,image/webp,image/svg+xml" onChange={(e) => { const file = e.target.files?.[0]; if (file) upload.mutate(file, { onSuccess: (r) => setUrl(r.url) }); }} />
      ...
      <div className="flex justify-end gap-2"><Button variant="ghost" onClick={onClose}>Cancel</Button><Button disabled={!url || upload.isPending} onClick={() => { onInsert(url, alt); onClose(); }}>Insert</Button></div>
    </div>
  </div>;
}
